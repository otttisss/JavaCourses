...
public class Solution {
    ...
    public static boolean isWeekend(LocalDateTime dateTime) {
        ...
        int compareDay = dateTime.getDayOfWeek().getValue();
        int compareHour = dateTime.getHour();

        if (compareDay < FRIDAY && compareHour < WEEKEND_START_FRIDAY_CUT_OFF_HOUR)
            return false;
        else if (compareDay == FRIDAY && compareHour >= WEEKEND_START_FRIDAY_CUT_OFF_HOUR ||
        compareDay == SUNDAY && compareHour < WEEKEND_END_SUNDAY_CUT_OFF_HOUR ||
        compareDay == SATURDAY){
            return true;
        }
